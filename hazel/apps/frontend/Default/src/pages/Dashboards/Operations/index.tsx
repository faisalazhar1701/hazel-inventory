...
import { dashboardsAPI, OperationsDashboard } from '../../../api/dashboards';
import { warehousesAPI } from '../../../api/warehouses';
import { toast } from 'react-toastify';
import FeatherIcon from 'feather-icons-react';

const OperationsDashboardPage: React.FC = () => {
  document.title = 'Operations Dashboard | Hazel Inventory';
  ...
  const loadData = async () => {
    try {
      ...
      const data = await dashboardsAPI.getOperationsDashboard(filters);
      setDashboard(data);
    } catch (err) {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Operations Dashboard" pageTitle="Dashboards" />
                ...
                <Card>
                  <CardBody>
                    <div className="text-center py-5">
                      ...
                      <p className="mt-2">Loading operations dashboard...</p>
                    </div>
                  </CardBody>
                </Card>
              </Col>
            ...
            <>
                      ...
                      <div className="d-flex align-items-center">
                        <div className="flex-grow-1">
                          ...
                          <h4 className="mb-0 mt-2">
                            {formatHours(dashboard.averageFulfillmentTimeHours)}
                          </h4>
                        </div>
                    ...
                    <CardBody>
                      <div className="d-flex align-items-center">
                        <div className="flex-grow-1">
                          ...
                          <h4 className={`mb-0 mt-2 ${dashboard.fulfillmentRate >= 90 ? 'text-success' : dashboard.fulfillmentRate >= 75 ? 'text-warning' : 'text-danger'}`}>
                            {dashboard.fulfillmentRate.toFixed(1)}%
                          </h4>
                        </div>
                        <div className="flex-shrink-0">
                          <div className="avatar-sm">
                            <span className={`avatar-title rounded fs-2 ${dashboard.fulfillmentRate >= 90 ? 'bg-success-subtle' : dashboard.fulfillmentRate >= 75 ? 'bg-warning-subtle' : 'bg-danger-subtle'}`}>
                              <FeatherIcon icon="check-circle" className={dashboard.fulfillmentRate >= 90 ? 'text-success' : dashboard.fulfillmentRate >= 75 ? 'text-warning' : 'text-danger'} />
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <CardBody>
                      <div className="d-flex align-items-center">
                        <div className="flex-grow-1">
                          ...
                          <h4 className={`mb-0 mt-2 ${dashboard.cancellationRate < 5 ? 'text-success' : dashboard.cancellationRate < 10 ? 'text-warning' : 'text-danger'}`}>
                            {dashboard.cancellationRate.toFixed(1)}%
                          </h4>
                        </div>
                        <div className="flex-shrink-0">
                          <div className="avatar-sm">
                            <span className={`avatar-title rounded fs-2 ${dashboard.cancellationRate < 5 ? 'bg-success-subtle' : dashboard.cancellationRate < 10 ? 'bg-warning-subtle' : 'bg-danger-subtle'}`}>
                              <FeatherIcon icon="x-circle" className={dashboard.cancellationRate < 5 ? 'text-success' : dashboard.cancellationRate < 10 ? 'text-warning' : 'text-danger'} />
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <CardBody>
                      <div className="d-flex align-items-center">
                        <div className="flex-grow-1">
                          ...
                          <h4 className={`mb-0 mt-2 ${dashboard.returnRate < 5 ? 'text-success' : dashboard.returnRate < 10 ? 'text-warning' : 'text-danger'}`}>
                            {dashboard.returnRate.toFixed(1)}%
                          </h4>
                        </div>
                        <div className="flex-shrink-0">
                          <div className="avatar-sm">
                            <span className={`avatar-title rounded fs-2 ${dashboard.returnRate < 5 ? 'bg-success-subtle' : dashboard.returnRate < 10 ? 'bg-warning-subtle' : 'bg-danger-subtle'}`}>
                              <FeatherIcon icon="rotate-ccw" className={dashboard.returnRate < 5 ? 'text-success' : dashboard.returnRate < 10 ? 'text-warning' : 'text-danger'} />
                            </span>
                          </div>
                        </div>
                      </div>
              ...
              <Row className="mb-4">
                <Col>
                  <Card>
                    ...
                    <CardBody>
                      {dashboard.fulfillmentByChannel.length === 0 ? (
                        <div className="text-center py-5">
                        ...
                        <div className="table-responsive">
                          <Table className="table-nowrap align-middle mb-0">
                            ...
                            <tbody>
                              {dashboard.fulfillmentByChannel.map((item) => (
                                <tr key={item.channel}>
                                  <td>
              ...
              <Row>
                <Col>
                  <Card>
                    <CardHeader>
                      <h5 className="card-title mb-0">Warehouse Performance</h5>
                    </CardHeader>
                    <CardBody>
                      {dashboard.warehousePerformance.length === 0 ? (
                        <div className="text-center py-5">
                          <div className="text-muted mb-3">
                            <FeatherIcon icon="package" size={64} />
                          </div>
                          <h5>No Data Available</h5>
                          <p className="text-muted">No warehouse performance data found for the selected filters.</p>
                        </div>
                      ) : (
                        <div className="table-responsive">
                          <Table className="table-nowrap align-middle mb-0">
                            ...
                            <tbody>
                              {dashboard.warehousePerformance.map((warehouse) => (
                                <tr key={warehouse.warehouseId}>
                                  ...
                                  <td className="text-end">
                                    <Badge color="info" className="fs-6">
                                      {warehouse.ordersFulfilled.toLocaleString()}
                                    </Badge>
                                  </td>
                                  <td className="text-end">
                                  ...
                                  <td className="text-end fw-medium">
                                    {formatHours(warehouse.averageFulfillmentTimeHours)}
                                  </td>
                                </tr>
                              ))}
                            </tbody>
